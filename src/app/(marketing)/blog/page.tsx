...
export default async function Blog() {
  // getBlogPosts is synchronous; no need to await
  const allPosts = getBlogPosts();
  // Sort by publishedAt if present, else date, newest first
  const articles = allPosts.sort((a, b) => {
    const bd = new Date(b.publishedAt || b.date || 0).getTime();
    const ad = new Date(a.publishedAt || a.date || 0).getTime();
    return bd - ad;
  });
      ...
      <div className="min-h-[50vh] bg-white/50 shadow-[inset_10px_-50px_94px_0_rgb(199,199,199,0.2)] backdrop-blur-lg">
        <div className="mx-auto grid w-full max-w-screen-xl grid-cols-1 gap-8 px-2.5 py-10 lg:px-20 lg:grid-cols-3">
          {articles.map((data, idx) => (
            <BlogCard key={data.slug ?? idx} data={data} priority={idx <= 1} />
          ))}
        </div>
      </div>
    </>
